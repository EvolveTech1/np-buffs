...
AddEventHandler("np-polychair:startSitting", function()
  if not inRestaurantBuffZone then return end
  if (isSitting == true) then return end
  isSitting = true
  Citizen.CreateThread(function()
    local minuteCount = 0
...
AddEventHandler("np-polychair:stopSitting", function()
  if not inRestaurantBuffZone then return end
  ...
  Citizen.CreateThread(function()
    ...
    while isSitting and inRestaurantBuffZone do
      if minuteCount >= 5 then
        local percent = tonumber(string.format("%.2f", math.min(1.0, (minuteCount - 4) / 5)))
        TriggerEvent("np-buffs:applyBuff", "restaurant_double", nil)
      end
      Citizen.Wait(60000)
      minuteCount = minuteCount + 1
    end
